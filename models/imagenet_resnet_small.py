...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes_after_prune, planes_expand, planes_before_prune, index, bn_value, stride=1,
                 downsample=None, first_conv_key=None):
        ...
        # for aligning the output first conv and the first stage/layer
        self.first_conv_key = first_conv_key
    ...
    def forward(self, x):
        ...
        if x.size(1) != self.bn_value.size(1) and self.downsample is None:

            assert self.first_conv_key is not None

            #print('residual.size() ', residual.size(), 'self.bn_value.size() ', self.bn_value.size())

            #tmp = torch.zeros(size=(x.size(0), self.bn_value.size(1), x.size(2), x.size(3))).cuda()
            tmp = torch.zeros(size=(x.size(0), self.bn_value.size(1), x.size(2), x.size(3))).to(x.device)
            
            #tmp.index_add_(1, self.first_conv_key.cuda(), residual)
            tmp.index_add_(1, self.first_conv_key.to(x.device), residual)
        ...
        residual += self.bn_value.to(residual.device)
        residual.index_add_(1, self.index.to(residual.device), out)
...
class Bottleneck(nn.Module):
    ...
    def forward(self, x):
        residual = x
        out = self.conv1(x)
        out = self.bn1(out)  # the activation of bn1 && bn2 of pruned filters are not dealt? 
        out = self.relu(out)
        ...
        if self.downsample is not None:
            # the skip layer (downsample) is not pruned
            residual = self.downsample(x)
        ...
        # setting: with index match
        # calculate the bn activation of those pruned filters in advance.
        # And save in bn_value
        # for downsample layer, residual.out_channels >= out.out_channels

        # NOTE: bn_value is the original activation value of bn3 of input 0 
        # of those pruned filters
        #residual += self.bn_value.cuda() # to deal with the bn_3
        #residual.index_add_(1, self.index.cuda(), out)

        residual += self.bn_value.to(residual.device) # to deal with the bn_3
        residual.index_add_(1, self.index.to(residual.device), out)
...
class ResNet_small(nn.Module):
    ...
    def __init__(self, block, layers, use_bottleneck, index, bn_value,
                 num_for_construct=[64, 64, 64 * 4, 128, 128 * 4, 256, 256 * 4, 512, 512 * 4], first_conv_key=None,
                 num_classes=1000):
        ...
        self.no_first_downsample = 1
        #self.no_first_downsample = 0
        
        self.use_bottleneck = use_bottleneck  # res50 has a more downsample than res34/18 in the first stage
        #self.use_bottleneck = 0
        #self.use_bottleneck = 1
        ...
        print('self.no_first_downsample = ', self.no_first_downsample)
        ...
        self.layer1 = self._make_layer(block, num_for_construct[1], num_for_construct[2], 64, self.index_layer1, self.bn_layer1,
                                       layers[0], stride=1, first_conv_key=first_conv_key)
    ...
    def _make_layer(self, block, planes_after_prune, planes_expand, planes_before_prune, index, bn_layer, blocks,
                    stride=1, first_conv_key=None):
        downsample = None
        if stride != 1 or self.inplanes != planes_before_prune * block.expansion:
        #if stride != 1 or self.inplanes != planes_after_prune * block.expansion:
        
            downsample = nn.Sequential(
                # see for details https://github.com/he-y/soft-filter-pruning/issues/2
                # downsample is skipped, not pruned, the out-channel of downsample layer is selected, not pruned
                # iden(x) + conv(x), only the conv 
                # would be pruned
            ...
            if self.no_first_downsample == 1:
                self.no_first_downsample = 0
                downsample = None
        

        print('downsample ', downsample is None)
        print(stride, self.inplanes, planes_before_prune, block.expansion)
        ...
        index_block_0_dict = {key: index[key] for key in index.keys() if '0.conv'+self.flag in key}
        ...
        layers.append(
            block(self.inplanes, planes_after_prune, planes_expand, planes_before_prune, index_block_0_value,
                  bn_layer_0_value,
                  stride, downsample, first_conv_key))
        ...
        for i in range(1, blocks):
            ...
            index_block_i_dict = {key: index[key] for key in index.keys() if (str(i) + '.conv'+self.flag) in key}
            ...
            bn_layer_i = {key: bn_layer[key] for key in bn_layer.keys() if (str(i) + '.bn'+self.flag) in key}
...
def resnet18_small(pretrained=False, **kwargs):
    ...
    """
    use_bottleneck = 0
    model = ResNet_small(BasicBlock, [2, 2, 2, 2], use_bottleneck, **kwargs)
    if pretrained:
...
def resnet34_small(pretrained=False, **kwargs):
    ...
    """
    use_bottleneck = 0
    model = ResNet_small(BasicBlock, [3, 4, 6, 3],  use_bottleneck, **kwargs)
    if pretrained:
...
def resnet50_small(pretrained=False, **kwargs):
    ...
    """
    use_bottleneck = 1
    model = ResNet_small(Bottleneck, [3, 4, 6, 3], use_bottleneck, **kwargs)
    if pretrained:
