...
def main(mask_0_decay, manualSeed, manual_lambda_hard, use_resume=False):
    ...
    m.init_mask(comp_rate) # 暂时，beta 是 一个常量，可以设计 成 随着 epoch增加 而 逐渐 增大 的 数字
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train(train_loader, model, m, criterion, optimizer, epoch, log)
...
class Mask:
    ...
    def get_filter_codebook_gradient_decay(self, weight_torch,compress_rate,length, index):
            ...
            if (index in self.mask_index):
                ...
                self.mat[index], self.mat_hard[index], self.filter_codebook[name] =  \
                       self.get_filter_codebook_gradient_decay(item.data, 
                                                ...
                                                self.model_length[index], index ) # 动量剪枝 需要 传入 index
